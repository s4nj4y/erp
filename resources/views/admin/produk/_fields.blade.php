...
<div class="grid md:grid-cols-2 gap-4">
    ...
    <div>
        <label class="block text-sm font-medium mb-1">UMKM</label>
        <select name="umkm_id" required class="w-full rounded-md border-gray-300 text-sm">
            <option value="">— pilih UMKM —</option>
            @foreach ($umkmList as $u)
                <option value="{{ $u->id }}" @selected(old('umkm_id', $p?->umkm_id) == $u->id)>{{ $u->nama_umkm }}</option>
            @endforeach
        </select>
    </div>
    <div>
...
<div class="grid md:grid-cols-2 gap-4">
    <div>
        ...
        @if ($p?->gambar)
            <img src="{{ asset('storage/'.$p->gambar) }}" class="h-16 w-16 object-cover rounded mb-2">
        @endif
