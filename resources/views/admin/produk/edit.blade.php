<x-admin-layout header="Edit Produk">
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm h-fit">
            ...
            <form method="POST" action="{{ route('admin.produk.stok.store', $produk) }}" class="space-y-2 mb-4">
                @csrf
            ...
            <ul class="divide-y divide-gray-100 text-sm max-h-64 overflow-auto">
                @forelse ($produk->stoks->sortByDesc('tanggal') as $s)
                    <li class="py-2 flex items-center justify-between">
                        <div>
                            <span class="{{ $s->status === 'masuk' ? 'text-green-600' : 'text-red-500' }}">
                                {{ $s->status === 'masuk' ? '+'.$s->jumlah_masuk : '-'.$s->jumlah_keluar }}
                            </span>
                            <span class="text-xs text-gray-400 ml-1">{{ $s->tanggal->format('d/m/Y') }}</span>
                        </div>
                        <form action="{{ route('admin.produk.stok.destroy', $s) }}" method="POST" onsubmit="return confirm('Hapus catatan stok ini?')">
                            @csrf @method('DELETE')
