<x-umkm-layout header="Produk">
    <div class="flex items-center justify-between mb-4">
        ...
        <a href="{{ route('umkm.produk.create') }}" class="px-4 py-2 bg-emerald-600 text-white rounded-md text-sm">+ Tambah Produk</a>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        <table class="w-full text-sm">
            <thead class="bg-gray-50 text-left text-gray-500">
                <tr>
                    <th class="px-4 py-3">Produk</th><th class="px-4 py-3 text-right">Harga</th>
                    <th class="px-4 py-3 text-center">Stok</th><th class="px-4 py-3">Tampil</th>
                    ...
                    <tr>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                <div class="h-10 w-10 bg-gray-100 rounded overflow-hidden flex items-center justify-center text-gray-300 text-xs">
                        ...
                        <td class="px-4 py-3">
                            <form action="{{ route('umkm.produk.toggle', $p) }}" method="POST">@csrf @method('PATCH')
                                <button class="{{ $p->show ? 'text-green-600' : 'text-gray-400' }} hover:underline">{{ $p->show ? 'Ya' : 'Tidak' }}</button>
                            </form>
                        </td>
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            <a href="{{ route('umkm.produk.edit', $p) }}" class="text-emerald-600 hover:underline">Edit</a>
                            <form action="{{ route('umkm.produk.destroy', $p) }}" method="POST" class="inline" onsubmit="return confirm('Hapus produk ini?')">@csrf @method('DELETE')<button class="text-red-600 hover:underline ml-2">Hapus</button></form>
                        </td>
                    </tr>
                @empty
